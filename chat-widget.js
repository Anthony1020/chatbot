...
(function() {
    ...
    const styles = `
        ...
        .n8n-chat-widget .chat-container {
            ...
            overflow: hidden;
            font-family: inherit;
			max-width: 95vw; /* Added for mobile */		
			max-height: 90vh; /* Added for mobile */
        }
	...
	function saveSessionToStorage(sessionId, messages) {
    ...
    async function startNewConversation() {
		...
		if (existingSession.sessionId) {
			...
			saveSessionToStorage(currentSessionId, []);
			...
			try {
				...
				const messages = [{type: 'bot', content: botMessage}];
	...
	async function sendMessage(message) {
		...
		try {
			...
			botMessage = botMessage.replace(urlRegex, function(url) {
			...
			messages.push({type: 'bot', content: botMessage});
			saveSessionToStorage(currentSessionId, messages);
	...
	// Optional: Add a function to check if the chat session exists
	function checkExistingSession() {
		const session = loadSessionFromStorage();
		if (session.sessionId) {
			startNewConversation(); // This will restore the existing session
		}
	}

	// Call this function when the widget is initialized
	window.addEventListener('load', checkExistingSession);

	// Optional: Add a clear chat button
	const addClearChatButton = () => {
		...
		clearButton.addEventListener('mouseout', () => clearButton.style.opacity = '0.8');
		chatFooter.appendChild(clearButton);
	};

	// Call this after the widget is created
	addClearChatButton();
